...
bool ImGui::Checkbox(const char* label, bool* v, float red, float green, float blue)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(red, green, blue, it_anim->second.animation), false, 9.0f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, float red, float green, float blue)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(red, green, blue), 5.0f);
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, float red, float green, float blue)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, red, green, blue);
}
